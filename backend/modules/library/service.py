...
from backend.modules.library.schemas import LibraryFile, LibraryFolder
from pdf_processing.parser import make_document_id


def build_tree(library_path: Path, db: Session) -> LibraryFolder:
    """Рекурсивно проходит папку библиотеки и возвращает дерево."""
    # Грузим все документы из БД один раз и матчим по slug.
    # Позже, когда добавим Document.source_path, можно матчить по пути.
    docs_by_slug = {doc.slug: doc for doc in db.scalars(select(Document)).all()}
    return _walk(library_path, docs_by_slug)
