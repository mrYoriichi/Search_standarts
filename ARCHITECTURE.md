...
down to the page. Written by a bridge engineer learning software engineering,
and used daily in a design office.
...
answer. These PDFs are not plain text: inside them are scans, construction
schemes and tables. The search has to handle both kinds of query, the exact
designation (a standard code, a sheet number) and the question asked by
meaning. Drawings have to be searchable as well. And all of it has to stay
confidential: engineers cannot hand their documents and finished projects to
a third party or upload them to somebody else's server.
...
provides the confidentiality. Retrieval is hybrid: embeddings for meaning,
BM25 for exact terms and codes, merged by reciprocal rank fusion. The
language model never does the searching; it describes drawings while indexing
and phrases the final answer out of the fragments retrieval found.
...
thousands of fragments. Nothing to install, nothing to run, nothing to break —
which matters when the product is a desktop installer, not a service.
...
be in different languages; the answer language is a separate setting, and
query expansion uses the languages actually present in the filtered corpus.
...
**Page router** (`pdf_processing/page_router.py`). A page is a drawing if
vector geometry dominates (>1000 PATH objects) or there is no extractable
text (<50 characters). Measured on live data: prose pages reach 575 paths,
drawings run 3 200–117 000 — a margin of at least 7×, frozen by a test.

**Prose pages** go through Docling (a temporary PDF of the prose pages only,
...
truncated at embedding time. Tables arrive as markdown cell text with a
vision retelling on top.
...
"passport" of the sheet, and become one fragment per page. The division of
labour matters: the vision model supplies semantics only, exact title-block
strings come from OCR, and the design stage is extracted with a regex —
...
| Memory | ~140 KB peak per fragment → 3000 pages ≈ 4500 fragments ≈ 630 MB peak |

The memory figure is where the page cap of the public build comes from: the
peak (JSON parsing) is roughly twice the steady state and has to stay safe on
an 8 GB laptop.
